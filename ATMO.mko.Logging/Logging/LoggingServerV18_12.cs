...
namespace ATMO.mko.Logging.Logging
{
    ...
    public class LoggingServerV18_12 : ILoggingServer
    {
        ...
        bool _AsyncOutput = true;

        /// <summary>
        /// Meldungen werden ab jetzt asynchron protokolliert. Hierdurch blockiert das Schreiben auf das Protokollmedium
        /// nicht mehr den aufrufenden Prozess. Jedoch kann die Reihenfolge der Meldungen im Protokollmedium von der 
        /// tatsächlichen Reihenfolge abweichen.
        /// </summary>
        public void AsyncOutputOn()
        {
            _AsyncOutput = true;
        }

        /// <summary>
        /// Meldungen werden ab jetzt synchron protokolliert. Hierdurch blockiert das Schreiben auf das Protokollmedium
        /// den aufrufenden Prozess. Die Reihenfolge der Meldungen im Protokollmedium stimmt mit der tatsächlichen Reihenfolge
        /// der Meldungen exakt überein.
        /// </summary>
        public void AsyncOutputOff()
        {
            _AsyncOutput = false;
        }
        ...
        /// <summary>
        /// mko, 10.3.2020
        /// Implementierung der Log- Methode verallgemeinert
        /// </summary>
        /// <param name="logType"></param>
        /// <param name="docuEntity"></param>
        /// <param name="SessionId"></param>
        ...
        private void WriteToStream(Info info, Action<ILogInfo18_12> AppendToStream)
        {
            if (AppendToStream != null)
            {
                ...
                for (int i = 0; i < invocationList.Length; i++)
                {
                    ...
                    if (_AsyncOutput)
                    {
                        // Asynchroner Aufruf der Eventhandler
                        Task.Run(() => ((Action<ILogInfo18_12>)invocationList[ii])(info));
                    }
                    else
                    {
                        // Snchroner Aufruf der Eventhandler- stellt sicher, das die Ausgaben auf dem 
                        // Protokollmedium in der richtigen Reihenfolge geschrieben werden (z.B. Console)
                        ((Action<ILogInfo18_12>)invocationList[ii])(info);
                    }
                }
            }
        }
