...
namespace MkPrgNet.Pattern.Automaton.Test
{
    ...
    public class MooreTests
    {
        ...
        public void Drehkreuz_anlegen()
        {
            ...
            // mko, 12.8.2019
            // Fehlerzustand als Default- Zustand definieren
            states.DefineStateAsDefault(Drehkreuz.Err);
            ...
            // 12.8.2019
            // Anstatt einen vollumfänglichen Definition aller Zustandsübergänge nur die wichtigen definieren
            transitions.DefTransistionFor(Drehkreuz.Zu, Zahlen, Drehkreuz.Auf);
            transitions.DefTransistionFor(Drehkreuz.Zu, AmKreuz, Drehkreuz.Zu);
            transitions.DefTransistionFor(Drehkreuz.Auf, AmKreuz, Drehkreuz.Zu);
            transitions.DefTransistionFor(Drehkreuz.Auf, Zahlen, Drehkreuz.Auf);
            transitions.DefTransistionFor(Drehkreuz.Aus, Zahlen, Drehkreuz.Aus);
            transitions.DefTransistionFor(Drehkreuz.Aus, AmKreuz, Drehkreuz.Aus);

            // Deaktiviert, um Übergang in Default- Zustand zu prüfen
            //transitions.DefTransistionFor(Drehkreuz.Zu, Aus, Drehkreuz.Aus);

            transitions.DefTransistionFor(Drehkreuz.Auf, Aus, Drehkreuz.Aus);
            transitions.DefTransistionFor(Drehkreuz.Aus, Aus, Drehkreuz.Aus);
        ...
        public void Drehkreuz_testen()
        {
            ...
            // Prüfung auf Default- Zustand
            Assert.AreEqual(Drehkreuz.Err, DrehkreuzAutomat.CurrentState);
            Assert.IsTrue(DrehkreuzAutomat.StateProperties.IsFinal(Drehkreuz.Err));
            Assert.IsTrue(DrehkreuzAutomat.StateProperties.IsStart(Drehkreuz.Zu));
