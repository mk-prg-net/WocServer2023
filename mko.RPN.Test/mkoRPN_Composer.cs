...
namespace mko.RPN.Test
{
    ...
    public class mkoRPN_Composer
    {
        ...
        [TestMethod]
        public void mkoRPN_Encoding()
        {

            var txt = "#ver 1.0.0";

            var encoded = mko.RPN.UrlSaveStringEncoder.RPNUrlSaveStringEncodeIf(txt, true);
            var decoded = mko.RPN.UrlSaveStringEncoder.RPNUrlSaveStringDecodeIf(encoded, true);

            Assert.AreEqual(txt, decoded);

            txt = "#ver     1.0 .0";

            encoded = mko.RPN.UrlSaveStringEncoder.RPNUrlSaveStringEncodeIf(txt, true);
            decoded = mko.RPN.UrlSaveStringEncoder.RPNUrlSaveStringDecodeIf(encoded, true);
