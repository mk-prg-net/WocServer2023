                ...
                if (!streamReader.EndOfStream)
                {
                    ...
                    } else
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (TryParseFunctionName(rawToken, out IToken token))
                                {
                                    _currentToken = token;
                                }
                                else if (long.TryParse(rawToken, out long lngValue))
                                {
                                    _currentToken = new IntToken(lngValue);
                                }
                                else if (double.TryParse(rawToken, out double dblValue))
                                {
                                    _currentToken = new DoubleToken(dblValue);
                                }
                                else if (bool.TryParse(rawToken, out bool boolValue))
                                {
