       ...
       /// <returns></returns>
        public RC<Result> Parse(IToken[] Tokens)
        {
            ...
            try
            {
                ...
                //IToken token = inStack.Any() ? inStack.Pop(): null;

                for (ixToken = 0; inStack.Any() && rc.Succeeded; ixToken++)
                {
                        ...
                        if (FuncEvaluators.ContainsKey(funcname))
                        {
                            ...
                            else
                            {
                                ...
                                if (stack.Count > paramCount && !(stack.Peek() is ListEndToken))
                                {
                                    ...
                                    // In nachfolgenden Schleifenzyklen sind diese auszuwerten, hier 
                                    // wäre es noch zu früh, da unbekannt ist, ob die soeben erzeugten Parameter
                                    // nicht wiederum Genertoren sind

                                    var bufferStack = new Stack<IToken>(stack.Count - paramCount);
                                    ...
                                    TokenBuffer.Add(new FunctionNameToken(token.Value, 1));
                                }
                                else
                                {
                                    // Die Anzahl der Paramter hat sich vermindert (Reducer). 
                                    // Die Ausmaß der Reduktion wird protokolliert
